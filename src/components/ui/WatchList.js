...
export function WatchList() {
    ...
    return (
        <div class="mt-10 p-2 rounded-md">
            <h2 class="text-2xl font-bold mb-4">Mi Lista de Películas:</h2>
            {movies2.length === 0 ? (
                ...
                <ul class="space-y-4">
                    {movies2.map((movie) => (
                        <li key={movie.id} class="flex jusfity-between items-center p4 rounded shadow-md">
                           <span class="font-semibold text-xl">{movie.title} </span> 
                            <div class="ml-auto">
                                <button onClick={()=>handleClear(movie)} class="hover:text-red-700 text-3xl font-medium">❌</button>
                            </div>
